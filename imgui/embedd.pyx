...
# distutils: sources = imgui-cpp/imgui_draw.cpp imgui-cpp/imgui_demo.cpp imgui-cpp/imgui_widgets.cpp imgui-cpp/imgui_tables.cpp config-cpp/py_imconfig.cpp
...
cdef class _ImGuiContext(object):
    cdef ImGuiContext* _ptr

    @staticmethod
    cdef from_ptr(ImGuiContext* ptr):
        if ptr == NULL:
            return None
...
cdef class _DrawList(object):
    ...
    """
    cdef ImDrawList* _ptr

    @staticmethod
    cdef from_ptr(ImDrawList* ptr):
        if ptr == NULL:
            return None
    ...
    @flags.setter
    def flags(self, ImDrawListFlags flags):
    ...
    def add_polyline(
        ...
        cdef ImVec2 *pts
        pts = <ImVec2 *>malloc(num_points * cython.sizeof(ImVec2))
    ...
    def prim_rect(self, float a_x, float a_y, float b_x, float b_y, ImU32 color = 0xFFFFFFFF):
    ...
    def prim_write_vtx(self, float pos_x, float pos_y, float u, float v, ImU32 color = 0xFFFFFFFF):
        """Write a vertex
    ...
    def prim_write_idx(self, ImDrawIdx idx):
        """Write index
    ...
    def prim_vtx(self, float pos_x, float pos_y, float u, float v, ImU32 color = 0xFFFFFFFF):
...
cdef class GuiStyle(object):
    ...
    @staticmethod
    cdef GuiStyle from_ref(ImGuiStyle& ref):
    ...
    cdef GuiStyle _create():
        cdef ImGuiStyle* _ptr = new ImGuiStyle()
    ...
    def window_menu_button_position(self, ImGuiDir value):
        self._check_ptr()
    ...
    def color_button_position(self, ImGuiDir value):
        self._check_ptr()
    ...
    def anti_aliased_lines(self, bool value):
        self._check_ptr()
    ...
    def anti_aliased_line_use_tex(self, bool value):
        self._check_ptr()
    ...
    def anti_aliased_fill(self, bool value):
        self._check_ptr()
    ...
    def color(self, ImGuiCol variable):
        if not (0 <= variable < enums.ImGuiCol_COUNT):
            raise ValueError("Unknown style variable: {}".format(variable))
...
cdef class _ImGuiTableColumnSortSpecs(object):
    cdef ImGuiTableColumnSortSpecs* _ptr
    ...
    @staticmethod
    cdef from_ptr(ImGuiTableColumnSortSpecs* ptr):
        if ptr == NULL:
            return None
    ...
    def column_user_id(self, ImGuiID column_user_id):
        self._require_pointer()
    ...
    @column_index.setter
    def column_index(self, ImS16 column_index):
        self._require_pointer()
    ...
    @sort_order.setter
    def sort_order(self, ImS16 sort_order):
        self._require_pointer()
    ...
    def sort_direction(self, ImGuiSortDirection sort_direction):
        self._require_pointer()
...
cdef class _ImGuiTableColumnSortSpecs_array(object):
    ...
    @staticmethod
    cdef from_ptr(ImGuiTableSortSpecs* ptr):
        if ptr == NULL:
            return None
    ...
    cdef _get_item(self, size_t idx):
        ...
        cdef size_t offset = idx*sizeof(ImGuiTableColumnSortSpecs)
        cdef size_t pointer = <size_t>self._ptr.Specs + offset
        return _ImGuiTableColumnSortSpecs.from_ptr(<ImGuiTableColumnSortSpecs *>pointer)
...
cdef class _ImGuiTableSortSpecs(object):
    ...
    @specs_dirty.setter
    def specs_dirty(self, bool specs_dirty):
        self._require_pointer()
...
cdef class _ImGuiViewport(object):
    ...
    @staticmethod
    cdef from_ptr(ImGuiViewport* ptr):
        if ptr == NULL:
            return None
...
cdef class _StaticGlyphRanges(object):
    cdef const ImWchar* ranges_ptr

    @staticmethod
    cdef from_ptr(const ImWchar* ptr):
        if ptr == NULL:
            return None
...
cdef class _FontAtlas(object):
    ...
    """
    cdef ImFontAtlas* _ptr

    def __init__(self):
        pass

    @staticmethod
    cdef from_ptr(ImFontAtlas* ptr):
        if ptr == NULL:
            return None
    ...
    def add_font_from_file_ttf(
        ...
        self._require_pointer()
        # note: cannot use ImWchar here due to Cython bug
        # note: whole unicode
...
cdef class _IO(object):
    ...
    @config_flags.setter
    def config_flags(self, ImGuiConfigFlags value):
    ...
    def backend_flags(self, ImGuiBackendFlags value):
    ...
    def font_allow_user_scaling(self, bool value):
    ...
    def config_mac_osx_behaviors(self, bool value):
    ...
    def config_cursor_blink(self, bool value):
    ...
    def config_drag_click_to_input_text(self, bool value):
    ...
    def config_windows_resize_from_edges(self, bool value):
    ...
    def config_windows_move_from_title_bar_only(self, bool value):
    ...
    def mouse_draw_cursor(self, bool value):
...
cdef int _ImGuiInputTextCallback(ImGuiInputTextCallbackData* data):
...
cdef class _ImGuiInputTextCallbackData(object):
    ...
    cdef ImGuiInputTextCallbackData* _ptr

    def __init__(self):
        pass

    @staticmethod
    cdef from_ptr(ImGuiInputTextCallbackData* ptr):
        if ptr == NULL:
            return None
...
cdef void _ImGuiSizeCallback(ImGuiSizeCallbackData* data):
...
cdef class _ImGuiSizeCallbackData(object):
    ...
    cdef ImGuiSizeCallbackData* _ptr

    def __init__(self):
        pass

    @staticmethod
    cdef from_ptr(ImGuiSizeCallbackData* ptr):
        if ptr == NULL:
            return None
...
def get_style():
    return GuiStyle.from_ref(GetStyle())
...
def get_version():
    ...
    .. wraps::
        void GetVersion()
    """
    cdef const char* c_string = GetVersion()
...
def style_colors_classic(GuiStyle dst = None):
    ...
    """
    if dst:
        StyleColorsClassic(dst._ptr)
    else:
        StyleColorsClassic(NULL)
...
def style_colors_light(GuiStyle dst = None):
    ...
    """
    if dst:
        StyleColorsLight(dst._ptr)
    else:
        StyleColorsLight(NULL)
...
def begin(str label, closable=False, ImGuiWindowFlags flags=0):
    """Begin a window.
    ...
    return Begin(_bytes(label), &opened if closable else NULL, flags), opened
...
def get_draw_data():
    ...
    """
    return _DrawData.from_ptr(GetDrawData())
...
def get_content_region_max():
    ...
    """
    return _cast_ImVec2_tuple(GetContentRegionMax())
...
def get_content_region_available():
    ...
    .. wraps::
        ImVec2 GetContentRegionMax()
    """
    return _cast_ImVec2_tuple(GetContentRegionAvail())
...
def get_window_content_region_min():
    ...
    """
    return _cast_ImVec2_tuple(GetWindowContentRegionMin())
...
def get_window_content_region_max():
    ...
    .. wraps::
        ImVec2 GetWindowContentRegionMin()
    """
    return _cast_ImVec2_tuple(GetWindowContentRegionMax())
...
def set_window_focus_labeled(str label):
    ...
    """
    SetWindowFocus(_bytes(label))

def set_window_size(
    float width, float height, ImGuiCond condition=ONCE):
    """Set window size
    ...
    **Note:** usage of this function is not recommended. prefer using
    ...
    """
    SetWindowSize(_cast_args_ImVec2(width, height), condition)

def set_window_size_named(str label, float width, float height, ImGuiCond condition = ONCE):
...
def set_next_window_collapsed(
    bool collapsed, ImGuiCond condition=ALWAYS
):
    ...
    """
    SetNextWindowCollapsed(collapsed, condition)
...
def get_window_draw_list():
    ...
    """
    return _DrawList.from_ptr(GetWindowDrawList())
...
def get_overlay_draw_list():
    ...
    .. wraps::
        ImDrawList* GetWindowDrawList()
    """
    return _DrawList.from_ptr(GetOverlayDrawList())
...
def get_window_position():
    ...
    .. wraps::
        ImVec2 GetWindowPos()
    """
    return _cast_ImVec2_tuple(GetWindowPos())
...
def get_window_size():
    ...
    """
    return _cast_ImVec2_tuple(GetWindowSize())
...
def set_next_window_position(
    ...
    """
    SetNextWindowPos(_cast_args_ImVec2(x, y), condition, _cast_args_ImVec2(pivot_x, pivot_y))


def set_next_window_size(
    float width, float height, ImGuiCond condition=ALWAYS
):
    ...
    """
    SetNextWindowSize(_cast_args_ImVec2(width, height), condition)
...
def set_next_window_size_constraints(
    ...
    """
    cdef ImGuiSizeCallback _callback = NULL
...
def set_next_window_content_size(float width, float height):
    ...
    """
    SetNextWindowContentSize(_cast_args_ImVec2(width, height))

def set_window_position(float x, float y, ImGuiCond condition = ALWAYS):
    ...
    """
    SetWindowPos(_cast_args_ImVec2(x,y), condition)

def set_window_position_labeled(str label, float x, float y, ImGuiCond condition = ALWAYS):
...
def set_window_collapsed(bool collapsed, ImGuiCond condition = ALWAYS):
    ...
    """
    SetWindowCollapsed(collapsed, condition)

def set_window_collapsed_labeled(str label, bool collapsed, ImGuiCond condition = ALWAYS):
    ...
    """
    SetWindowCollapsed(_bytes(label), collapsed, condition)
...
def tree_node(str text, ImGuiTreeNodeFlags flags=0):
    """Draw a tree node.
    ...
    """
    return TreeNodeEx(_bytes(text), flags)
...
def collapsing_header(
    ...
    """
    cdef bool inout_opened = visible
    if visible is None:
        clicked = CollapsingHeader(_bytes(text), NULL, flags)
    else:
        clicked = CollapsingHeader(_bytes(text), &inout_opened, flags)
    return clicked, None if visible is None else inout_opened

def set_next_item_open(bool is_open, ImGuiCond condition = 0):
    ...
    """
    SetNextItemOpen(is_open, condition)
...
def selectable(
    ...
    """
    cdef bool inout_selected = selected
    return Selectable(
        _bytes(label),
        &inout_selected,
        flags,
...
def begin_menu(str label, enabled=True):
    ...
    """
    return BeginMenu(_bytes(label), enabled)
...
def menu_item(
    str label, str shortcut=None, bool selected=False, enabled=True
):
    """Create a menu item.
...
def open_popup(str label, ImGuiPopupFlags flags=0):
    """Open a popup window.
    ...
    """
    OpenPopup(_bytes(label), flags)

def open_popup_on_item_click(str label = None, ImGuiPopupFlags popup_flags = 1):
    ...
    """
    if label is None:
        OpenPopupOnItemClick(NULL, popup_flags)
    else:
        OpenPopupOnItemClick(_bytes(label), popup_flags)


def begin_popup(str label, ImGuiWindowFlags flags=0):
    """Open a popup window.
    ...
    """
    return BeginPopup(_bytes(label), flags)


def begin_popup_modal(str title, visible=None, ImGuiWindowFlags flags=0):
    ...
    """
    cdef bool inout_visible = visible
...
def begin_popup_context_item(str label = None, ImGuiPopupFlags mouse_button = 1):
    ...
    """
    if label is None:
        return BeginPopupContextItem(NULL, mouse_button)
    else:
        return BeginPopupContextItem(_bytes(label), mouse_button)
...
def begin_popup_context_void(str label = None, ImGuiPopupFlags popup_flags = 1):
    ...
    if label is None:
        return BeginPopupContextVoid( NULL, popup_flags )
    else:
        return BeginPopupContextVoid( _bytes(label), popup_flags )

def is_popup_open( str label,  ImGuiPopupFlags flags = 0):
    ...
    * ``is_popup_open()`` with POPUP_ANY_POPUP_ID + POPUP_ANY_POPUP_LEVEL: return true if any popup is open.
    ...
    """
    return IsPopupOpen(_bytes(str), flags)
...
def table_get_sort_specs():
    ...
    """
    cdef ImGuiTableSortSpecs* imgui_sort_spec = TableGetSortSpecs()
...
def table_get_column_name(int column_n = -1):
    ...
    """
    return _from_bytes(TableGetColumnName(column_n))
...
def text_colored(str text, float r, float g, float b, float a=1.):
    ...
    """
    # note: "%s" required for safety and to favor of Python string formating
    TextColored(_cast_args_ImVec4(r, g, b, a), "%s", _bytes(text))
...
def label_text(str label, str text):
    ...
    """
    # note: "%s" required for safety and to favor of Python string formating
    LabelText(_bytes(label), "%s", _bytes(text))
...
def button(str label, width=0, height=0):
    ...
    """
    return Button(_bytes(label), _cast_args_ImVec2(width, height))
...
def small_button(str label):
    ...
    """
    return SmallButton(_bytes(label))

def arrow_button(str label, ImGuiDir direction = DIRECTION_NONE):
    ...
    return ArrowButton(_bytes(label), direction)

def invisible_button(str identifier, float width, float height, ImGuiButtonFlags flags = 0):
...
def checkbox(str label, bool state):
    ...
    """
    cdef bool inout_state = state
    return Checkbox(_bytes(label), &inout_state), inout_state
...
def checkbox_flags(str label, unsigned int flags, unsigned int flags_value):
    ...
    return CheckboxFlags(_bytes(label), &inout_flags, flags_value), inout_flags


def radio_button(str label, bool active):
    ...
    """
    return RadioButton(_bytes(label), active)
...
def color_edit3(str label, float r, float g, float b, ImGuiColorEditFlags flags = 0):
...
def color_edit4(
    str label, float r, float g, float b, float a, bool show_alpha=True
):
...
def input_text(
    ...
    cdef ImGuiInputTextCallback _callback = NULL
    cdef void *_user_data = NULL
    if callback is not None:
...
def progress_bar(float fraction, size = (-FLOAT_MIN,0), str overlay = ""):
    ...
    """
    ProgressBar(fraction, _cast_tuple_ImVec2(size), _bytes(overlay))
...
def is_item_clicked(ImGuiMouseButton mouse_button = 0):
...
def get_item_rect_min():
    ...
    """
    return _cast_ImVec2_tuple(GetItemRectMin())
...
def get_item_rect_max():
    ...
    """
    return _cast_ImVec2_tuple(GetItemRectMax())
...
def get_item_rect_size():
    ...
    """
    return _cast_ImVec2_tuple(GetItemRectSize())
...
def get_main_viewport():
    ...
    """
    return _ImGuiViewport.from_ptr(GetMainViewport())
...
def is_rect_visible(float size_width, float size_height):
    ...
    """
    return IsRectVisible(_cast_args_ImVec2(size_width, size_height))
...
def get_style_color_name(int index):
    ...
    """
    cdef const char* c_string = GetStyleColorName(index)
    cdef bytes py_string = c_string
    return c_string.decode("utf-8")
...
def get_background_draw_list():
    ...
    """
    return _DrawList.from_ptr(GetBackgroundDrawList())
...
def get_foreground_draw_list():
    ...
    """
    return _DrawList.from_ptr(GetForegroundDrawList())
...
def set_mouse_cursor(ImGuiMouseCursor mouse_cursor_type):
    ...
    """
    return SetMouseCursor(mouse_cursor_type)
...
def get_clipboard_text():
    ...
    """
    return _from_bytes(GetClipboardText())
...
def load_ini_settings_from_disk(str ini_file_name):
    ...
    """
    LoadIniSettingsFromDisk(_bytes(ini_file_name))
...
def load_ini_settings_from_memory(str ini_data):
    ...
    LoadIniSettingsFromMemory(_bytes(ini_data), 0)
...
def save_ini_settings_to_disk(str ini_file_name):
    ...
    """
    SaveIniSettingsToDisk(_bytes(ini_file_name))
...
def save_ini_settings_to_memory():
    ...
    """
    return _from_bytes(SaveIniSettingsToMemory(NULL))
...
#    """
#    return SetScrollHere(center_y_ratio)
...
def color_convert_u32_to_float4(ImU32 in_):
    ...
    """
    return _cast_ImVec4_tuple(ColorConvertU32ToFloat4(in_))
...
def color_convert_float4_to_u32(float r, float g, float b, float a):
    ...
    """
    cdef ImVec4 color = _cast_args_ImVec4(r,g,b,a)
...
cpdef push_style_var(ImGuiStyleVar variable, value):
    ...
    **Note:** variables pushed on stack need to be poped using
    ...
    try:
        if isinstance(value, (tuple, list)):
            PushStyleVar(variable, _cast_tuple_ImVec2(value))
        else:
            PushStyleVar(variable, <float>(float(value)))
    except ValueError:
        raise ValueError(
...
cpdef push_style_color(
    ...
    **Note:** variables pushed on stack need to be popped using
    ...
    PushStyleColor(variable, _cast_args_ImVec4(r, g, b, a))
    return True
...
cpdef get_style_color_vec_4(ImGuiCol idx):
    return _cast_ImVec4_tuple(GetStyleColorVec4(idx))

cpdef get_font_tex_uv_white_pixel():
    return _cast_ImVec2_tuple(GetFontTexUvWhitePixel())
...
cpdef get_color_u32_idx(ImGuiCol idx, float alpha_mul = 1.0):
...
cpdef get_color_u32_rgba(float r, float g, float b, float a):
    ...
    """
    return GetColorU32( _cast_args_ImVec4(r, g, b, a) )


cpdef get_color_u32(ImU32 col):
    """retrieve given style color with style alpha applied and optional extra alpha multiplier
...
cpdef push_allow_keyboard_focus(bool allow_focus):
    PushAllowKeyboardFocus(allow_focus)
...
def dummy(width, height):
    ...
    """
    Dummy(_cast_args_ImVec2(width, height))
...
def columns(int count=1, str identifier=None, bool border=True):
    ...
    if identifier is None:
        Columns(count, NULL, border)
    else:
        Columns(count, _bytes(identifier), border)
...
def begin_tab_bar(str identifier, ImGuiTabBarFlags flags = 0):
    ...
    """
    return BeginTabBar(_bytes(identifier), flags)
...
def begin_tab_item(str label, opened = None, ImGuiTabItemFlags flags = 0):
    """Create a Tab.
    ...
    """
    cdef bool inout_opened = opened
    return BeginTabItem(_bytes(label), &inout_opened if opened is not None else NULL, flags), inout_opened
...
def tab_item_button(str label, ImGuiTabItemFlags flags = 0):
    ...
    """
    return TabItemButton(_bytes(label), flags)
...
def set_tab_item_closed(str tab_or_docked_window_label):
    ...
    """
    SetTabItemClosed(_bytes(tab_or_docked_window_label))

def begin_drag_drop_source(ImGuiDragDropFlags flags=0):
...
def set_drag_drop_payload(str type, bytes data, ImGuiCond condition=0):
    ...
    **Note:** this is a beta API.
    ...
    """
    return SetDragDropPayload(_bytes(type), <const char*>data, len(data), condition)
...
def accept_drag_drop_payload(str type, ImGuiDragDropFlags flags=0):
    ...
    **Note:** this is a beta API.
    ...
    """
    cdef const ImGuiPayload* payload = AcceptDragDropPayload(_bytes(type), flags)
    if payload == NULL:
        return None
...
def get_drag_drop_payload():
    ...
    """
    cdef const ImGuiPayload* payload = GetDragDropPayload()
    if payload == NULL:
        return None
    cdef const char* data = <const char *>payload.Data
    return <bytes>data[:payload.DataSize]
...
def get_cursor_pos():
    ...
    .. wraps::
        ImVec2 GetCursorPos()
    """
    return _cast_ImVec2_tuple(GetCursorPos())
...
def set_cursor_pos(local_pos):
    ...
    """
    SetCursorPos(_cast_tuple_ImVec2(local_pos))
...
def get_cursor_start_pos():
    ...
    """
    return _cast_ImVec2_tuple(GetCursorStartPos())
...
def get_cursor_screen_pos():
    ...
    """
    return _cast_ImVec2_tuple(GetCursorScreenPos())
...
def set_cursor_screen_pos(screen_pos):
    ...
    .. wraps::
        ImVec2 SetCursorScreenPos(const ImVec2& screen_pos)
    """
    SetCursorScreenPos(_cast_tuple_ImVec2(screen_pos))
...
def create_context(_FontAtlas shared_font_atlas = None):
    ...
    if (shared_font_atlas):
        _ptr = CreateContext(shared_font_atlas._ptr)
    else:
        _ptr = CreateContext(NULL)
...
@contextmanager
def _py_styled(ImGuiStyleVar variable, value):
...
def _py_vertex_buffer_vertex_pos_offset():
    return <uintptr_t><size_t>&(<ImDrawVert*>NULL).pos

def _py_vertex_buffer_vertex_uv_offset():
    return <uintptr_t><size_t>&(<ImDrawVert*>NULL).uv

def _py_vertex_buffer_vertex_col_offset():
    return <uintptr_t><size_t>&(<ImDrawVert*>NULL).col
